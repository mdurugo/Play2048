...
public class Board {
    ...
    public void updateOpenSpaces() {
        ...
        for(int r = 0; r < gameBoard.length; r++){
            for(int c = 0; c < gameBoard[r].length; c++){
                if(gameBoard[r][c] == 0){
                    BoardSpot spot = new BoardSpot(r, c);
                    openSpaces.add(spot);
                }
            }
        }
    ...
    public void addRandomTile() {
        ...
        int board = StdRandom.uniform(a, b);
        double upper = 1;
        double lower = 0;
        double randS = StdRandom.uniform(lower, upper);
        BoardSpot spot = openSpaces.get(board);

        gameBoard[spot.getRow()][spot.getCol()] = board;
        if(randS < 0.1){
    ...
    public void swipeLeft() {
        ...
        for(int r = 0; r < gameBoard.length; r++){
            ...
            for(int c = 0; c < gameBoard[r].length; c++){
                if(gameBoard[r][c] != 0){
                    gameBoard[r][temp++] = gameBoard[r][c];
                }
            }
            while(temp < gameBoard[r].length){
                gameBoard[r][temp++] = 0;
            }
        }
    }
    ...
    public void flipRows() {
        ...
        for(int i = 0; i < gameBoard.length; i++){
            ...
            while(j < gameBoard[i].length / 2){
                ...
                gameBoard[i][gameBoard[i].length - j - 1] = hold;
                j++;
            }
        }
    }
